...
//
    ...
    void LabelGraph::run(LabelGraph* g, bool isForward, std::vector<std::pair<int, std::vector<LABEL_TYPE>>> AncestorSet, int u, int v, LABEL_TYPE deleteLabel, std::promise<boost::unordered_set<int>> &promiseObj) {
        auto affectedNode = g->DeleteLabelWithMultiThread(isForward, AncestorSet, u, v, deleteLabel);
        promiseObj.set_value(affectedNode);
    }

    boost::unordered_set<int> LabelGraph::DeleteLabelWithMultiThread(bool isForward, std::vector<std::pair<int, std::vector<LABEL_TYPE>>> AncestorSet, int u, int v, LABEL_TYPE deleteLabel) {
        if (isForward) {
            boost::unordered_set<int> forwardAffectedNode;

            for (auto i = 0; i < AncestorSet.size(); i++) {
                int s = AncestorSet[i].first;
                DeleteLabel(s, v, AncestorSet[i].second, InLabel[v], true);

                std::queue<std::pair<int, LABEL_TYPE>> q;
                for (auto node : AncestorSet[i].second) {
                    q.push(std::make_pair(v, node));
                }
            ...
            for (auto i = 0; i < AncestorSet.size(); i++) {
                ...
                DeleteLabel(s, u, AncestorSet[i].second, OutLabel[u], false);
                std::queue<std::pair<int, LABEL_TYPE>> q;
                for (auto node : AncestorSet[i].second) {
                    q.push(std::make_pair(u, node));
                }
            ...
            return backwardAffectedNode;
        }
    }

    void LabelGraph::runFindPath(LabelGraph* g, bool isForward, boost::unordered_set<int> affectedNode, std::promise<std::pair<std::vector<std::tuple<int, int, LABEL_TYPE, EdgeNode *>>, std::vector<std::tuple<int, int, LABEL_TYPE, EdgeNode *>>>> &promiseObj) {
        auto result = g->FindPathMultiThread(isForward, affectedNode);
        promiseObj.set_value(result);
    }

    std::pair<std::vector<std::tuple<int, int, LABEL_TYPE, EdgeNode *>>, std::vector<std::tuple<int, int, LABEL_TYPE, EdgeNode *>>> LabelGraph::FindPathMultiThread(bool isForward, boost::unordered_set<int> affectedNode) {
        std::vector<std::tuple<int, int, LABEL_TYPE, EdgeNode *>> forwardPrunedPath;
        std::vector<std::tuple<int, int, LABEL_TYPE, EdgeNode *>> backwardPrunedPath;

        if (isForward) {
            for (auto i : affectedNode) {
        ...
        return std::make_pair(forwardPrunedPath, backwardPrunedPath);
    }
    ...
    LabelGraph::DeleteEdgeLabel(int u, int v, LABEL_TYPE &deleteLabel, boost::unordered_set<int> &forwardAffectedNode,
        ...
        // forward
        {
            ...
            int totalNum = InAncestorSet.size();
...
//            }
        ...
        // backward
        {
            ...
            int totalNum = OutAncestorSet.size();
            int subNum = std::max(totalNum / THREAD_NUM, 1);
...
//    t.StartTimer("Path");
        ...
        for (auto i=0;i<threadList.size();i++) {
            ...
            for (auto& j : result.second) {
                backwardPrunedPath.push_back(j);
            }
        }
...
// only used for minimal version
    bool LabelGraph::TryInsertWithoutInvUpdate(int s, int u, int v, LABEL_TYPE label, LABEL_TYPE curLabel,
        ...
        InOrOutLabel.insert(InOrOutLabel.begin() + left, LabelNode(s, u, curLabel, label, edge));
...
// label是边的label, curLabel是一个大的label
    bool LabelGraph::TryInsert(int s, int u, int v, LABEL_TYPE label, LABEL_TYPE curLabel,
        ...
        auto addNode = LabelNode(s, u, curLabel, label, edge);
    ...
    void LabelGraph::GenerateInvLabel() {
        for (int i = 0; i < InLabel.size(); i++) {
            for (auto &j : InLabel[i]) {
                InvInLabel[j.id][std::make_pair(i, j.label)] = LabelNode(i, j.lastID,
                                                                         j.label,
                                                                         j.lastLabel,
                                                                         j.lastEdge);
            }
        ...
        for (int i = 0; i < OutLabel.size(); i++) {
            for (auto &j : OutLabel[i]) {
                InvOutLabel[j.id][std::make_pair(i, j.label)] = LabelNode(i, j.lastID,
                                                                          j.label,
                                                                          j.lastLabel,
                                                                          j.lastEdge);
            }
